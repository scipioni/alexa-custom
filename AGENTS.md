...
- **Always use `pro-audio`** profile for the `NewPie` USB audio device (`alsa_card.usb-0a12_NewPie_SABINESMICDFU-00`).
- This is the **only profile** that exposes both a sink (`*.pro-output-0`) and a source (`*.pro-input-0`) simultaneously. Combined `output:analog-stereo+input:*` profiles are not generated by WirePlumber for this hardware.
- **Do not use `output:analog-stereo`** alone — it has `sources: 0`, leaving the microphone unavailable.
