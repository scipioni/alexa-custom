...
class SherpaOnnxSTT(STTBackend):
    def __init__(
        ...
        if model_variant == "auto":
        ...
        elif model_variant == "transducer":
            if not os.path.exists(joiner) and not os.path.exists(joiner_int8):
                raise RuntimeError(
                    f"sherpa-onnx: model_variant='transducer' but joiner.onnx not found in {model_dir!r}"
                )
            self._delegate = sherpa_onnx.OnlineRecognizer.from_transducer(
                tokens=tokens,
                encoder=encoder_int8 if os.path.exists(encoder_int8) else encoder,
        ...
        elif model_variant == "zipformer2-ctc":
            if not os.path.exists(model_onnx):
                raise RuntimeError(
                    f"sherpa-onnx: model_variant='zipformer2-ctc' but model.onnx not found in {model_dir!r}"
                )
            if not hasattr(sherpa_onnx.OnlineRecognizer, "from_zipformer2_ctc"):
                raise RuntimeError(
                    "sherpa-onnx: model_variant='zipformer2-ctc' but from_zipformer2_ctc "
                    "is not available in the installed sherpa_onnx version"
                )
            self._delegate = sherpa_onnx.OnlineRecognizer.from_zipformer2_ctc(
                tokens=tokens,
                model=model_onnx,
        ...
        elif model_variant == "nemo_ctc":
            if not os.path.exists(model_onnx):
                raise RuntimeError(
                    f"sherpa-onnx: model_variant='nemo_ctc' but model.onnx not found in {model_dir!r}"
                )
            if not hasattr(sherpa_onnx.OnlineRecognizer, "from_nemo_ctc"):
                raise RuntimeError(
                    "sherpa-onnx: model_variant='nemo_ctc' but from_nemo_ctc "
                    "is not available in the installed sherpa_onnx version"
                )
            self._delegate = sherpa_onnx.OnlineRecognizer.from_nemo_ctc(
                tokens=tokens,
                model=model_onnx,
                num_threads=4,
                sample_rate=16000,
            ...
            if not os.path.exists(encoder) and not os.path.exists(decoder):
                raise RuntimeError(
                    f"sherpa-onnx: model_variant='paraformer' but encoder.onnx/decoder.onnx not found in {model_dir!r}"
                )
            self._delegate = sherpa_onnx.OnlineRecognizer.from_paraformer(
                tokens=tokens,
                encoder=encoder,
...
class WhisperCppSTT(STTBackend):
    ...
    def __init__(
        self,
        model_path: str = _WHISPER_CPP_MODEL_PATH,
        num_threads: int = 4,
    ) -> None:
        try:
            import whisper_cpp
            ...
            raise RuntimeError(
                ...
                "Run the cross-compilation script at addons/whisper-cpp/ "
                "and pip install the produced wheel on the target board."
            )
        if not os.path.isfile(model_path):
            raise RuntimeError(
                f"whisper.cpp model not found at {model_path!r}. "
                f"Run 'alexa-setup --whisper-cpp' to download it."
            )
        self._model = whisper_cpp.Whisper(model_path, n_threads=num_threads)
    ...
    def finalize(self) -> str:
        ...
        self._result = (
            result["text"].strip() if isinstance(result, dict) else result.strip()
        )
        self._buffer.clear()
        return self._result
