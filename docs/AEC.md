...
| `noise_suppression` | Estimates a stationary noise floor per-frequency-bin and applies spectral subtraction | Lifts Vosk/sherpa-onnx confidence scores on clean speech by reducing the noise that competes with formants. Most effective on constant sources (fans, hum). |
