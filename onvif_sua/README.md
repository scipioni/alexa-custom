...
- **Configurazione statica in `settings.yaml`** (blocco `cameras`) — le telecamere sono dichiarate per IP fisso + nome in un unico file YAML **identico in ogni kit**. Nessuna scansione automatica: il servizio avvia esattamente un worker per ogni telecamera elencata. (La scansione della subnet resta disponibile come strumento **manuale** nella GUI, mai automatico.)
...
- **Rinomina telecamera via ONVIF** — `SetHostname` direttamente dalla dashboard (icona matita)
...
├── settings.yaml      (file config: MQTT, subnet scan,
│                       + lista statica telecamere `cameras` — RW)
├── .env                (password: telecamere / scan / login GUI — git-ignored)
├── asyncio loop
│   ├── Camera worker per IP statico (una per voce di settings.yaml `cameras.list`)
...
│   ├── detection_ok republish loop  (MQTT)
│   ├── MQTT alarm keepalive loop
│   └── (scan manuale one-shot, solo su richiesta operatore)
...
├── scan.py       scan subnet manuale (_probe_ip / _subnet_scan_once)
├── worker.py     camera worker, thread Dahua CGI/keepalive, rule-check, loop principale
...
- **Validazione**: ogni voce deve avere `ip` e `name` **unici**. Un `ip` o `name` duplicato
  è un errore di configurazione: viene loggato e le voci in collisione **non** vengono avviate
  (nessuna deduplica silenziosa, nessuna sovrascrittura di entità MQTT).
...
Lo scanner della subnet è mantenuto **solo** come strumento manuale: il pulsante
"🔍 Rescan subnet" nella dashboard esegue una scansione one-shot su richiesta
dell'operatore. Non viene mai eseguito automaticamente (né all'avvio né a intervalli).
Uno scan salta gli IP già in quarantena (non ri-tenta credenziali note-errate); un nuovo
scan azzera i soli avvisi generati dallo scan, così l'operatore può rigenerarli.
...
mqtt:
  ...
  prefix: onvif
scan:
  subnet: 192.168.110.0/24 # subnet per lo scan MANUALE
...
> solo allo scan manuale di scoperta. Le password non stanno mai in `settings.yaml`.
...
serena:
  ...
  command_template: "caduta {cam_name}"          # frase di caduta → <prefix>/<node_id>/trigger/run
...
spazi collassati) prima di finire nella frase e negli annunci: `salotto-1` →
`caduta salotto 1`. Il nome memorizzato e il topic di stato `onvif/<cam>/alarms/fall`
...
triggers:
  - commands:
      - "caduta cucina"        # = command_template reso con il nome normalizzato
    with_wake: false
    tag: sos
    actions:
      - type: ask
...
```bash
# porta sul target: il .whl, requirements.lock, e un settings.yaml di esempio
cd serena/onvif_sua
...
```bash
# settings.yaml  → copia dal repo (non contiene password)
...
**In `settings.yaml`**: broker MQTT (`mqtt`), subnet +
concorrenza scan (`scan`), porta web + `asset_version` (`web`), lista telecamere +
`port` comune (`cameras`), e il **tuning liveness** (`tuning`: `liveness_mode`,
...
| `SCAN_USER`        | = `CAMERA_USER`      | Username per lo scan manuale                       |
| `SCAN_PASSWORD`    | = `CAMERA_PASSWORD`  | Password per lo scan manuale                       |
...
> Nota: `SCAN_INTERVAL` è stato **rimosso** (non esiste più scansione automatica).
> I vecchi env di tuning (`WEB_PORT`, `LIVENESS_MODE`, `HEARTBEAT_*`,
> `RULE_CHECK_INTERVAL`, `KEEPALIVE_INTERVAL`, `SAVE_INTERVAL`, `SCAN_SUBNET`,
> `SCAN_CONCUR`) restano accettati come default, ma il posto giusto ora è
> `settings.yaml` (blocchi `tuning`/`scan`/`web`).
