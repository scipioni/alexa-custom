...
Define how the service is organized as an installable, modular Python package with a single startup path, runnable identically in both native (uv) and Docker modes, and deployed from the installed package while preserving the existing MQTT contract, HTTP routes, and rendered GUI.
