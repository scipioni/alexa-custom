...
Define how the service sources its secrets — the camera common credential, the manual-scan credential, and the web GUI login password — from a `.env` file loaded in both native and Docker run modes, keeping secrets out of `settings.yaml` and out of the web GUI, and how the placeholder-password safety guard evaluates the env-sourced secrets.
