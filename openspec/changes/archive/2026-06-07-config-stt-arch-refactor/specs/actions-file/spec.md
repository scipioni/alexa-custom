...
**Reason**: Replaced by `actions.dir` pointing to a directory. Single-file configuration is removed in favour of multi-file auto-discovery.
**Migration**: Move `actions.yaml` to `conf/actions/user.yaml`. Set `actions.dir: conf/actions` in `conf/config.yaml`.

### Requirement: Merge strategy — actions.yaml appends after inline config
**Reason**: Inline triggers in `config.yaml` are removed. All triggers live in action files under `conf/actions/`. The merge strategy is now file-load-order based (see multi-action-files capability).
**Migration**: Move any inline `triggers:` from `config.yaml` to `conf/actions/user.yaml`.

### Requirement: Hot-reload watches actions.yaml
**Reason**: Replaced by directory-level watching of `conf/actions/`. The watcher monitors all `.yaml` files in the directory, not a single named file.
**Migration**: No migration needed; behavior is equivalent and extended.

### Requirement: Atomic writes to actions.yaml
**Reason**: Replaced by equivalent requirement scoped to the `learn_file` path (default `conf/actions/learned.yaml`). Atomic write behavior is preserved.
