...
## Purpose
Keep the LiveKit worker loop running through transient and terminal failures so a single uncaught exception cannot silently kill call handling while the rest of the daemon (web dashboard, STT, audio watcher) stays up.
