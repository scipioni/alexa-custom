...
Build and publish `alexa-custom` (root `serena` project) and `onvif-sua` as
versioned wheels/sdists to a private, PyPI-compatible registry, so that
production boards can install released packages without a full git checkout.
Versioning and credential handling reuse existing project conventions
(`release:*` tasks, git-ignored secrets) rather than introducing new ones.
