...
Provision `alexa-custom` and `onvif-sua` onto a production board from a
standalone `production/` bundle — fetchable without a full `serena` git
checkout — covering package install, audio setup, local MQTT broker/bridge
setup, and `systemd --user` service installation, with a one-shot guided
bootstrap, a documented end-to-end procedure, a non-destructive uninstall,
and a drift guard keeping the bundle's forked setup scripts in sync with
their source-of-truth counterparts in the main repo.
