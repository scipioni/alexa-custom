...
Provides dark/light theme switching for the web dashboard. The theme is initialised from OS color-scheme preference, can be overridden by the user via a toggle button, and the choice is persisted to `localStorage` across page reloads. A synchronous inline script prevents any flash of incorrect theme on load.
