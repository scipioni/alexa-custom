#!/usr/bin/env bash
# Render mosquitto-bridge.conf.template into /etc/mosquitto/conf.d/,
# substituting the bridge credentials from a secrets file and this board's
# hostname (the remote namespace root — see docs/mqtt_integration.md).
#
# Forked from scripts/render-mqtt-bridge.sh — see SOURCES.md. The only
# intentional difference from that source of truth is TEMPLATE/SECRETS
# defaulting to this bundle's own layout (no serena checkout, no
# alexa-custom-repo-relative conf/secrets.yaml) instead of a repo checkout's
# paths. Everything else (rendering, substitution, error handling) is
# identical — keep it that way; task check:drift only special-cases this file
# because of that one default-path difference.
#
# The credentials cannot live in mosquitto-serena.conf (committed), and they
# cannot live in a separate conf.d drop-in either: mosquitto tracks the
# current bridge per config file, so a `username` line outside the file
# holding `connection ...` fails with "Error: Invalid bridge configuration".
# Hence a template rendered into a root-owned file at deploy time.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEMPLATE="$SCRIPT_DIR/mosquitto-bridge.conf.template"
SECRETS="${SERENA_SECRETS:-$HOME/apps/alexa-custom/conf/secrets.yaml}"
TARGET="${MQTT_BRIDGE_CONF:-/etc/mosquitto/conf.d/serena-bridge.conf}"

[[ -f "$TEMPLATE" ]] || { echo "Template not found: $TEMPLATE" >&2; exit 1; }
[[ -f "$SECRETS" ]] || {
  echo "Secrets file not found: $SECRETS" >&2
  echo "Run 'task config:init' first, then add mqtt.bridge_username / mqtt.bridge_password to it." >&2
  exit 1
}
...
for key in ("bridge_username", "bridge_password"):
    value = mqtt.get(key)
    if not value:
        sys.exit(f"{sys.argv[1]}: mqtt.{key} is missing or empty")
