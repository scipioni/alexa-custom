...
class TestLoadConfig:
    def test_env_section_writes_to_os_environ(self, tmp_path):
        cfg = write_file(tmp_path, "config.yaml",
...
# ---------------------------------------------------------------------------
# load_actions_config_auto tests (task 1.2)
# ---------------------------------------------------------------------------

class TestLoadActionsConfigAuto:
    def test_prefers_config_yaml_over_actions_yaml(self, tmp_path):
        write_file(tmp_path, "config.yaml",
            "env:\n  _TEST_SOURCE: config_yaml\n" + MINIMAL_ACTIONS)
        write_file(tmp_path, "actions.yaml", MINIMAL_ACTIONS)
        from alexa_custom.config import load_actions_config_auto
        result = load_actions_config_auto(base_dir=tmp_path)
        assert os.environ.get("_TEST_SOURCE") == "config_yaml"
        assert result is not None

    def test_falls_back_to_actions_yaml_with_warning(self, tmp_path, caplog):
        write_file(tmp_path, "actions.yaml", MINIMAL_ACTIONS)
        import logging
        from alexa_custom.config import load_actions_config_auto
        with caplog.at_level(logging.WARNING, logger="alexa_custom.config"):
            result = load_actions_config_auto(base_dir=tmp_path)
        assert result is not None
        assert any("deprecated" in r.message.lower() or "actions.yaml" in r.message for r in caplog.records)

    def test_returns_none_when_neither_file_present(self, tmp_path):
        from alexa_custom.config import load_actions_config_auto
        result = load_actions_config_auto(base_dir=tmp_path)
        assert result is None

    def test_config_yaml_takes_precedence_even_if_both_present(self, tmp_path):
        write_file(tmp_path, "config.yaml", """\
            wake_words: [custom]
            triggers: []
        """)
        write_file(tmp_path, "actions.yaml", """\
            wake_words: [alexa]
            triggers: []
        """)
        from alexa_custom.config import load_actions_config_auto
        result = load_actions_config_auto(base_dir=tmp_path)
        assert result is not None
        assert result.wake_words == ["custom"]
...
class TestConfigManager:
    def _make_config(self, tmp_path: Path, wake_words=None) -> Path:
        ...
        p.write_text(f"wake_words: {words}\ntriggers: []\n")
        return p
    ...
    def test_register_reload_callback_called_on_reload(self, tmp_path):
        ...
        cfg_path.write_text("wake_words: [computer]\ntriggers: []\n")
    ...
    async def test_watcher_detects_file_change_and_reloads(self, tmp_path):
        ...
        received = []
        mgr.register_reload_callback(lambda c: received.append(c.wake_words))
